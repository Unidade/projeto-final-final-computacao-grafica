...
{
    glEnable(GL_LIGHT0);
    GLfloat sceneAmbient[] = {0.045f, 0.045f, 0.06f, 1.0f};
    glLightModelfv(GL_LIGHT_MODEL_AMBIENT, sceneAmbient);
    GLfloat sunDiffuse[] = {0.12f, 0.12f, 0.18f, 1.0f};
...
void setupFlashlightOnce()
{
    ...
    glLightf(GL_LIGHT2, GL_CONSTANT_ATTENUATION, 0.85f);
    glLightf(GL_LIGHT2, GL_LINEAR_ATTENUATION, 0.22f);
    glLightf(GL_LIGHT2, GL_QUADRATIC_ATTENUATION, 0.18f);

    glLightf(GL_LIGHT2, GL_SPOT_CUTOFF, 14.0f);
    glLightf(GL_LIGHT2, GL_SPOT_EXPONENT, 42.0f);
}
